...
def create_target_labels(df, lookahead=5, threshold=0.03):
    """Create 4-class target labels based on future price movement"""
    df = df.copy()
    df['future_price'] = df['close'].shift(-lookahead)
    df['price_change'] = (df['future_price'] - df['close']) / df['close']
    df = df.dropna()
    
    # Create signal: 0=strong decline, 1=slight decline, 2=slight rise, 3=strong rise
    df['signal'] = 1  # Default
    df.loc[df['price_change'] < -threshold, 'signal'] = 0  # Strong decline
    df.loc[(df['price_change'] >= -threshold) & (df['price_change'] < 0), 'signal'] = 1  # Slight decline
    df.loc[(df['price_change'] > 0) & (df['price_change'] <= threshold), 'signal'] = 2  # Slight rise
    df.loc[df['price_change'] > threshold, 'signal'] = 3  # Strong rise
    
    # Count distribution
    signal_counts = df['signal'].value_counts().sort_index()
    print(f"Signal distribution - Strong Decline(0): {signal_counts.get(0, 0)}, "
          f"Slight Decline(1): {signal_counts.get(1, 0)}, "
          f"Slight Rise(2): {signal_counts.get(2, 0)}, "
          f"Strong Rise(3): {signal_counts.get(3, 0)}")
