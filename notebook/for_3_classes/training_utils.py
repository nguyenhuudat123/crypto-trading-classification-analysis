...
def calculate_directional_loss(y_true, y_pred, price_changes):
    """
    Calculate loss based on wrong directional predictions
    ...
    # Loss when predicting wrong direction
    wrong_predictions = ((y_pred == 0) & (price_changes > 0)) | ((y_pred == 2) & (price_changes < 0))
    loss_values[wrong_predictions] = np.abs(price_changes[wrong_predictions])
    
    transaction = np.sum((y_pred == 0) | (y_pred == 2))
